...
export default function FollowingScreen() {
  ...
  const fetchFollowing = async (p = 1, replace = false) => {
    try {
      ...
      const resp = (await apiService.getFollowing(id, p, 20)) as any
      const list: UserLite[] = resp?.users || []
      if (replace) setUsers(list)
      else setUsers((prev) => (p === 1 ? list : [...prev, ...list]))
  ...
  useEffect(() => {
    setLoading(true)
    Promise.all([loadMyFollowing(), fetchFollowing(1, true)])
      .then(() => {})
  ...
  const onRefresh = async () => {
    setRefreshing(true)
    await Promise.all([loadMyFollowing(), fetchFollowing(1, true)])
  }
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={!loading ? <View style={{ padding: 24, alignItems: 'center' }}><Text style={{ color: colors.muted }}>No following yet</Text></View> : null}
        />
      )}
    </View>
  )
